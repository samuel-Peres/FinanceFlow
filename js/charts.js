// ============================================
// GRÁFICOS AVANÇADOS - COMPLETO COM TEXTOS BRANCOS
// ============================================
const Charts = {
  ...
  renderExpenseChart() {
    ...
    if (Object.keys(categories).length === 0) {
      this.expenseChartInstance = new Chart(ctx, {
        type: 'doughnut',
        data: { labels: ['Sem dados'], datasets: [{ data: [1], backgroundColor: ['var(--text-tertiary)'] }] },
        options: { 
          responsive: true, 
  ...
  renderBankDistribution() {
    ...
    if (Object.keys(bankBalances).length === 0) {
      this.bankChartInstance = new Chart(ctx, { 
        type: 'pie', 
        data: { labels: ['Sem contas'], datasets: [{ data: [1], backgroundColor: ['var(--text-tertiary)'] }] },
        options: { 
          responsive: true, 
          maintainAspectRatio: false, 
          plugins: { 
  ...
  renderForecastChart() {
    ...
    this.forecastChartInstance = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          y: { 
            ticks: { callback: (v) => `R$ ${v}`, color: '#ffffff' }, 
            grid: { color: 'rgba(255,255,255,0.1)' } 
          },
          x: { 
